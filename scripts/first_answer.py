...
class Simulater:
    ...
    def evaluate(self,selected_jobs):
        ...
        for _ in range(self.times_test):
            suggest_cost=self.pape(self.edges,self.n_node,self.start_pos,selected_jobs)
...
def solve(epsilon,cooling_rate,epoch):
    ...
    while time.time()-start_time<limit:
        ...
        sum_score=sum([fix[i][0] for i in range(len(fix))])
        ...
        if score_a+score_b>1000000000000:  
            #rate=min(1*(1-(time.time()-fit_start)/fit_time),.8)
            rate=.8
        else:
            rate=0.15
        ...
        for i in range(size):
            temp_sel_a_list[i],temp_sel_b_list[i]=temp_sel_b_list[i],temp_sel_a_list[i]
        ...
        if temp_score<(score_a+score_b) or (rd.random()<burn and temp_score<10**9):
